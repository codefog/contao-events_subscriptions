...
class MemberConfig
{
    ...
    {
        return StringUtil::deserialize($this->member->groups, true);
    }
    ...
    {
        if ($this->member->subscription_enableLimit) {
            $data = StringUtil::deserialize($this->member->subscription_periodLimit, true);
        ...
        foreach ($this->groups as $group) {
            if ($group->subscription_enableLimit) {
                $data = StringUtil::deserialize($group->subscription_periodLimit, true);
    ...
    public static function create($memberId)
    {
        ...
        if (($groups = MemberGroupModel::findMultipleByIds(StringUtil::deserialize($member->groups, true))) === null) {
