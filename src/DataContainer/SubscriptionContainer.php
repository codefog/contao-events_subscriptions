...
use Codefog\EventsSubscriptionsBundle\EventDispatcher;
...
class SubscriptionContainer
{
     ...
     */
    public function displaySummary(DataContainer $dc)
    {
        // Return if not a list view
        if ((int)$dc->currentPid !== (int)Input::get('id') || Input::get('act') || Input::get('key')) {
            return;
        }

        try {
            $config = Services::getEventConfigFactory()->create($dc->currentPid);
        } catch (\Exception $e) {
            return;
        }
    ...
    public function dispatchSubscribeEvent(DataContainer $dc)
    {
        ...
        System::getContainer()->get('monolog.logger.contao')->log(
            sprintf($logMessage, strip_tags($subscription->getFrontendLabel()), $subscriptionModel->getEvent()->title, $subscriptionModel->getEvent()->id),
