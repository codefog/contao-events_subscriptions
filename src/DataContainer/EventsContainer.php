...
class EventsContainer
{
    ...
    {
        if (($calendarModel = CalendarModel::findByPk(Input::get('id'))) === null || !$calendarModel->subscription_enable) {
            return '';
        }

        return sprintf(
            '<a href="%s" class="%s" title="%s"%s>%s</a> ',
            str_replace('&amp;table=tl_calendar_events', '', Backend::addToUrl($href.'&amp;id='.Input::get('id'))),
            $class,
